...
#include <memory>
#include <limits>
#include "nav2_navigate_to_pose_with_planner/navigate_to_pose_with_planner.hpp"

namespace nav2_navigate_to_pose_with_planner
{

bool
NavigateToPoseNavigatorWithPlanner::configure(
  ...
  std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
{
  ...
  goal_sub_ = node->create_subscription<nav2_custom_msgs::msg::PoseStampedWithPlanner>(
    "goal_pose_with_planner",
    rclcpp::SystemDefaultsQoS(),
    std::bind(&NavigateToPoseNavigatorWithPlanner::onGoalPoseReceived, this, std::placeholders::_1));
  return true;
}

std::string
NavigateToPoseNavigatorWithPlanner::getDefaultBTFilepath(
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  std::string default_bt_with_planner_xml_filename;
  auto node = parent_node.lock();

  if (!node->has_parameter("default_nav_to_pose_with_planner_bt_xml")) {
    std::string pkg_share_dir =
      ament_index_cpp::get_package_share_directory("nav2_navigate_to_pose_with_planner");
  ...
  return default_bt_with_planner_xml_filename;
}

bool
NavigateToPoseNavigatorWithPlanner::cleanup()
{
  goal_sub_.reset();
  self_client_.reset();
  return true;
...
bool
NavigateToPoseNavigatorWithPlanner::goalReceived(ActionT::Goal::ConstSharedPtr goal)
{
  auto default_bt_with_planner_xml_filename = goal->behavior_tree;

  if (!bt_action_server_->loadBehaviorTree(default_bt_with_planner_xml_filename)) {
    RCLCPP_ERROR(
      logger_, "BT file not found: %s. Navigation canceled.",
      default_bt_with_planner_xml_filename.c_str());
    return false;
  }
...
void
NavigateToPoseNavigatorWithPlanner::goalCompleted(
...
void
NavigateToPoseNavigatorWithPlanner::onLoop()
{
...
void
NavigateToPoseNavigatorWithPlanner::onPreempt(ActionT::Goal::ConstSharedPtr goal)
{
...
void
NavigateToPoseNavigatorWithPlanner::initializeGoalPose(ActionT::Goal::ConstSharedPtr goal)
{
  geometry_msgs::msg::PoseStamped current_pose;
  nav2_util::getCurrentPose(
    current_pose, *feedback_utils_.tf,
...
void NavigateToPoseNavigatorWithPlanner::onGoalPoseReceived(const nav2_custom_msgs::msg::PoseStampedWithPlanner::SharedPtr pose_with_planner)
{
  ActionT::Goal goal;
  goal.pose = pose_with_planner->pose;
  
  // Assuming the planner ID is a string, you can set it like this
  goal.planner_id = pose_with_planner->planner_id;

  // Now, send the goal to the action server asynchronously
...
PLUGINLIB_EXPORT_CLASS(
  nav2_navigate_to_pose_with_planner::NavigateToPoseNavigatorWithPlanner,
